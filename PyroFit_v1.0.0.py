...
def extract_datatype(filename):
	...
	"""
	if filename.endswith("ELT-Curr-t-I.log") or filename.endswith("ELT-Curr-t-I-VB.log") and 'Baseline' not in filename:
		return "Current"
	elif filename.endswith("ELT-Volt-t-V.log") and 'Baseline' not in filename:
		return "Voltage"
	elif filename.endswith("ELT-Char-t-Q.log") and 'Baseline' not in filename:
		return "Charge"
	elif filename.endswith("TEMP-t-Tpelt-Tsoll-Tsample.log") and 'Baseline' not in filename:
		return "Temperature"
	elif filename.endswith("PWR-t-I-U.log") and 'Baseline' not in filename:
		return "Powersupply"
	elif filename.endswith("VAC-t-pressure.log") and 'Baseline' not in filename:
		return "Vacuum"
	elif filename.endswith("HiV-t-HVsetVoltage-HVmeasVoltage.log") and 'Baseline' not in filename:
		return "HighVoltage"
	elif filename.endswith("GPIB-t-addr-errcount-cmd-data-atconvdat-STBerrque.log") and 'Baseline' not in filename:
		return "GBIP-Errors"
	else:
		return None
...
def saving_figure(bild, pbild=False):
	...
	return None
def Lissajous(ax, x, y):
				from mpl_toolkits.axes_grid1.inset_locator import inset_axes
				axins = inset_axes(ax, width="15%", height="20%", loc=2)
				axins.set_xticklabels([])
				axins.set_yticklabels([])
				axins.plot(x,y,color=tubafcyan(),linestyle='-')
...
def rel_err(Tfit, Terror, Ifit, Ierror, area, area_error, phasediff, Xsigma=1):
	...
	phasediff_error = abs(Terror[2])+abs(Ierror[2])
...
if filelist == []:
	...
	if measurement_info['hv_mode'] == "Off":
		...
		#SineWave Method
			...
			Lissajous(ax1, Tnew[start_index:,0], Inew[start_index:])
		...
		#SineWave+LinearRamp Method
			...
			if answer == "y":
				...
				if BaselineCorrection == True:
					timeshift = float(prompt("timedelay [s]?"))
					#timeshift = 0.21
					tnew = tB+timeshift
					Inew = Inew-IB
					...
					ax2.lines.remove(ax2.lines[0])
					ax2.set_ylim(min(Inew),max(Inew))
				...
				if usecoolrate_flag == True:
					...
					satzlaenge = int(limit/T_perioden)
				else:
					number_of_lim = maxT_ind.tolist().count(True)
					...
					tmax = tnew[limit]
					satzlaenge = int((limit-start_index)/T_perioden)
				...
				if PartWiseTFit == True:
					...
					Tparams_down.add('phase', value=0.1, vary=True,min=0,max=2*pi)
				...
				for i in arange(1,I_perioden):
					...
					nonpyroparams = amp_phase_correction(nonpyroparams)
					ax2.plot(tnew[start:ende], sinfunc(nonpyroparams, tnew[start:ende]-tnew[start]), color=np_color,linestyle='-')
					
					#Calculating Data from Fit - TSC
					if calculate_data_from_fit_flag == True:
						TSC = (array([tnew[start:ende], sinfunc(nonpyroparams, tnew[start:ende]-tnew[start])])).T		#transpose!
						if i==1:
							I_TSC = TSC
						else:
					...
					pyroparams = amp_phase_correction(pyroparams)
					ax2.plot(tnew[start:ende], sinfunc(pyroparams, tnew[start:ende]-tnew[start]), color=p_color,linestyle='-')
					
					#Calculating Data from Fit - Pyro
					if calculate_data_from_fit_flag == True:
						pyro = (array([tnew[start:ende], sinfunc(pyroparams, tnew[start:ende]-tnew[start])])).T		#transpose!
						if i==1:
							I_pyro = pyro
						else:
					...
					if PartWiseTFit == True:
						Temp = tnew[int(satzlaenge/2)] * Tfit_down[i-1,4] + Tfit_down[i-1,3]
		...
		#SineWave+TriangleHat
			...
			if answer == "y":
				...
				if PartWiseTFit == True:
					...
					Tparams_down_heat.add('freq', value=measurement_info['freq'], min=0.003, max=0.005, vary=False)
					Tparams_down_heat.add('phase', value=0.1, vary=True, min=0, max=2*pi)
					...
					Tparams_down_cool.add('freq', value=measurement_info['freq'], min=0.003, max=0.005, vary=False)
					Tparams_down_cool.add('phase', value=0.1, vary=True, min=0, max=2*pi)
					...
					for i in arange(1,T_perioden):
						...
						if i < T_perioden_heat:	
							#heating				
							Tresult_sin_heat = minimize(sinfunc, Tparams_down_heat, args=(tnew[start:ende]-tnew[start], Tnew[start:ende,0]), method="leastsq")
							Tparams_down_heat = amp_phase_correction(Tresult_sin_heat.params)
							ax1.plot(tnew[start:ende], sinfunc(Tparams_down_heat, tnew[start:ende]-tnew[start]), color=temp_color, linestyle='-')
						...
						# fit for cooling
							...
							Tresult_sin_cool = minimize(sinfunc, Tparams_down_cool, args=(tnew[start:ende]-tnew[start], Tnew[start:ende,0]), method="leastsq")
							Tparams_down_cool = amp_phase_correction(Tresult_sin_cool.params)
							ax1.plot(tnew[start:ende], sinfunc(Tparams_down_cool, tnew[start:ende]-tnew[start]), color=temp_color, linestyle='-')
				...
				Iparams.add('phase', value=1.0, min=-pi, max=pi)
				Iparams.add('offs', value=1e-10)
				Iparams.add('slope', value=1e-10)
				...
				for i in arange(1,I_perioden):
					...
					ax2.plot(tnew[start:ende], sinfunc(Iparams, tnew[start:ende]-tnew[start]), 'r-')
					ax2.plot(tnew[start:ende], linear(Iparams_lin, tnew[start:ende]-tnew[start]), 'r--')
					...
					if i < T_perioden_heat:
						if PartWiseTFit == True:
							Temp = tnew[int(satzlaenge/2)] * Tfit_down_heat[i-1,4] + Tfit_down_heat[i-1,3]
					...
					#for cooling branche
						...
						if PartWiseTFit == True:
							Temp = tnew[int(satzlaenge/2)] * Tfit_down_cool[i-1-T_perioden_heat,4] + Tfit_down_cool[i-1-T_perioden_heat,3]
