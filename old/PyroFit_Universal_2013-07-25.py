...
#		-correct filename for figures in SineWave+LinRamp
...
if HV_status == "Off":
	...
	#SineWave Method
		...
		savefig((date+samplename).replace(".log", ".png"))
		draw()
	...
	#SineWave+LinearRamp Method
		...
		if input == "y":
			...
			#file output
			log = open(date+"_"+samplename+"_Fits.txt", 'w+')
			...
			bild1.savefig(name.replace(".log", "_data-fit.png"))
			bild2.savefig(name.replace(".log", "_pyro.png"))
			
			print "...saving figures"
			
			log_name2 = date + "_" + filename + "_Pyro-Pol-Phasediff-Data.txt"
