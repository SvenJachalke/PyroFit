...
upper_I_lim = 1e-8
sample_thickness = 0.5 #mm
temp_filter_flag = True
start_index = 100
phase_correction_flag = True
...
def extract_fit_relerr_params(params, fit, err):
	"""
	Extract the fitted parameters from the Paramters Dict and put it in a list (values and errors)
...
#given frequency and amplitude of the temperature stimulation
f = np.array([1,5,10,20,30,40,50])
A = np.array([0.5,1,2])
#measured phase difference between current and temperature
#for 0.5mm thick LTO
p_05 = array([[270.63,272.07,277.57,283.74,289.7,296.16,302.77],
			[269.96,273.04,278.59,284.92,290.32,292.9,300.79],
			[270.77,274.34,278.88,294.29,290.93,294.29,300.26]])
#for 1mm thick LTO
p_1 = array([[272.10,280.37,290.39,300.92,309.88,315.39,321.83],
			[272.04,282.95,292.70,303.10,311.21,318.35,319.79],
			[272.37,283.95,292.88,302.93,310.22,315.35,321.01]])
#for 1mm thick LTO
#p_2 = array([[272.10,280.37,290.39,300.92,309.88,315.39,321.83],
#			[272.04,282.95,292.70,303.10,311.21,318.35,319.79],
#			[272.37,283.95,292.88,302.93,310.22,315.35,321.01]])

#actual interpolation
calibration_func_d05mm = interp2d(f,A,p_05)
calibration_func_d1mm = interp2d(f,A,p_1)
#calibration_func_d2mm = interp2d(f,A,p_2)

#new "grid" vor x- and y-axis
#f_new = arange(1,50,0.1)   #from 1 to 50mHz in 0.1 steps
#A_new = arange(0.1,2,0.05)   #from 0.1 to 2 K in 0.05 steps

#new values vor phase
#p_05_new = calibration_func_d05mm(f_new,A_new)
#p_1_new = calibration_func_d1mm(f_new,A_new)
#p_2_new = calibration_func_d2mm(f_new,A_new)
...
for filename in filelist:
	...
	if datatype=="Temperature":
		...
		#check on second PT100
		temp_filter_flag = False
		...
		#previous filter of data
		current_filter_flag = False
		strangerlist = []
		for i in range(len(Idata)):
			if Idata[i,1] == 0.015 or Idata[i,1] == 9.9E+39:
				strangerlist.append(i)
				current_filter_flag = True
		Idata = delete(Idata,strangerlist,0)
		filecounter = filecounter + 1
		sys.stdout.write("\rReading: %d/%d completed" % (filecounter,len(filelist)))
...
#-------------------------------------------------------------------------------------------------------------------------------------
	...
	if HV_status == "Off":
		...
		#SineWave Method
			...
			tnew = arange(min(Idata[:,0]),max(Tdata[:,0]),0.5)			#arange new time axis in 0.5s steps
			Tinterpol_down = interp1d(Tdata[:,0],Tdata[:,1])				#interpolation of lower temperature
			Tnew_down = Tinterpol_down(tnew)									#caluculate new lower temperature values
			if temp_filter_flag == False:								#the same for higher temperature, when filter flag is off
				Tinterpol_top = interp1d(Tdata[::5,0],Tdata[::5,3])
				Tnew_top = Tinterpol_top(tnew[:-5])
			
			#Interpolation current data
			Iinterpol = interp1d(Idata[:,0],Idata[:,1])
			Inew = Iinterpol(tnew)

			#filtering of data for currente values larger than "upper_I_lim"-value (low-pass-filter)
			ausreisserliste = []										#initialize filter list
			for i in arange(0,len(Inew)-1):
				if Inew[i]>=upper_I_lim:								#check Inew Array for values larger than upper_I_lim
					ausreisserliste.append(i)							#found values will be add to filter list
			Inew = delete(Inew,ausreisserliste,0)						#deleting from arrays!
			tnew = delete(tnew,ausreisserliste,0)
			Tnew_down = delete(Tnew_down,ausreisserliste,0)
			if temp_filter_flag == False:
				Tnew_top = delete(Tnew_top,ausreisserliste,0)
			...
			ax1.set_ylabel('temperature [K]',color='b',size='20')
			ax1.grid(b=None, which='major', axis='both', color='grey', linewidth=1)
			ax1.plot(tnew[start_index:], Tnew_down[start_index:], "bo", label='T meas. (Down)')
			if temp_filter_flag == False:
				ax1.plot(tnew[start_index:-5], Tnew_top[start_index:], "go", label='T meas. (Top)')

			#Plot Current
			ax2.set_ylabel('current [A]',color='r',size='20')
			ax2.autoscale(enable=True, axis='y', tight=None)
			ax2.plot(tnew[start_index:], Inew[start_index:], 'ro', label='I meas.')
			...
			if input == "y":
				...
				Tfit_low = [0,0,0,0,0]	#bottom temperature
				...
				Tparams_low = Parameters()
				Tparams_low.add('amp', value=start_parameters[0], min=0.1, max=40.0)
				Tparams_low.add('freq', value=start_parameters[1], min=1e-5, max=0.1, vary=False)
				Tparams_low.add('phase', value=0.1, min=-pi, max=pi)
				Tparams_low.add('offs', value=start_parameters[2], min=273., max=423.)
				Tparams_low.add('slope', value=start_parameters[3])

				#fit of bottom temperature
				Tresults = minimize(sinfunc, Tparams_low, args=(tnew[start_index:], Tnew_down[start_index:]), method="leastsq")
				extract_fit_relerr_params(Tparams_low, Tfit_low, Terror_low)		#write fit params to Tfit-list

				#correction of phase < 0
				Tfit_low[2] = PhaseRangeCheck(Tfit_low[2])

				#for top temperature
				if temp_filter_flag == False:
					#initialize parameters dict for lmfit (top temperature)
					Tparams_high = Parameters()
					Tparams_high.add('amp', value=start_parameters[0], min=0.1, max=40.0)
					Tparams_high.add('freq', value=start_parameters[1], min=1e-5, max=0.1, vary=False)
					Tparams_high.add('phase', value=0.1, min=-pi, max=pi)
					Tparams_high.add('offs', start_parameters[2], min=273., max=423.)
					Tparams_high.add('slope', value=start_parameters[3])

					#fit of top temperature
					Tresults2 = minimize(sinfunc, Tparams_high, args=(tnew[start_index+(len(tnew)/2):-5], Tnew_down[start_index+(len(tnew)/2):]), method="leastsq")
					extract_fit_relerr_params(Tparams_high, Tfit_high, Terror_high)

					#data corrections
					Tfit_high[2] = PhaseRangeCheck(Tfit_high[2])

				#plot of fits
				ax1.plot(tnew[start_index:], sinfunc(Tparams_low, tnew[start_index:]), 'b-', label='T-Fit (Down)')
				if temp_filter_flag == False:
				...
				Iparams = Parameters()
				Iparams.add('amp', value=1e-9, min=0.0)
				...
				extract_fit_relerr_params(Iparams, Ifit, Ierror) 	#extract params dict
				...
				ax2.plot(tnew[start_index:], sinfunc(pyroparams, tnew[start_index:]), 'c-', label='rI$_{p}$')
		...
		#SineWave+LinearRamp Method
			...
			if input == "y":
				...
				Tparams.add('offs', value=start_parameters[2], min=0.0)
