...
##User Settings-------------------------------------------------------------------------------------------------------------
matplotlib.rcParams['legend.fancybox'] = True #fancy box
upper_I_lim = 1e-6 #high pass filter limit for current
starting_point = 50 #start index for plotting and fits
...
def set_interpol_range(data1, data2):
	"""
	Function to find upper and lower time limit for the interpolation
	input: data [array]
	output: interpol_range [list]
	"""
	interpol_range = [0,0] #lower, upper t
	if data1[0,0] > data2[0,0]:
		interpol_range[0] = data1[0,0]
	else:
		interpol_range[0] = data2[0,0]

	if data1[-1,0] > data2[-1,0]:
		interpol_range[1] = data2[-1,0]
	else:
		interpol_range[1] = data1[-1,0]

	return interpol_range
...
#-------------------------------------------------------------------------------------------------------------------------------------
	...
	if HV_status == "Off":
		...
		#SineWave Method
			...
			#Interpolate data--------------------------------------
			#getting interpolation range
			interpol_range = set_interpol_range(Idata, Tdata)

			#new time
			tnew = arange(interpol_range[0],interpol_range[1],0.5)
			...
			if input == "y":
				...
				Tparams = Parameters()
				Tparams.add('amp', value=start_parameters[0], min=0.1, max=2)
				Tparams.add('freq', value=start_parameters[1], min=1e-5, max=0.1, vary=False)
				Tparams.add('phase', value=0.1, min=-pi, max=pi)
				Tparams.add('offs', value=start_parameters[3])
				Tparams.add('slope', value=start_parameters[4])

				#fit of bottom temperature
				Tresults = minimize(sinfunc, Tparams, args=(tnew[start_index:-2], Tnew1[start_index:-2]), method="leastsq")
				...
				if temp_filter_flag == False:
					...
					Tparams2.add('offs', start_parameters[2], min=273)
