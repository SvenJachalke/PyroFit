...
#-------------------------------------------------------------------------------------------------------------------------------------
	...
	if HV_status == "Off":
		...
		if T_profile == "Thermostat":
			...
			print "--------------------------------"
			print "...saving figure"
			savefig(date+'_'+samplename+'_Thermostat.png')
		...
		#LinearRamp Method
			...
			print "--------------------------------"
			print "...saving figure"
			savefig(date+'_'+samplename+'_LinRamp.png')
		...
		#SineWave Method
			...
			if input == "y":
				...
				if shape(Tdata)[1]>=4 and max(Tdata[:,3])!=9.9e+37:
					...
					Tresults2 = minimize(sinfunc, Tparams2, args=(tnew[start_index:], Tnew2[start_index:]), method="leastsq")
				...
				if shape(Tdata)[1]>=4 and max(Tdata[:,3])!=9.9e+37:
					ax1.plot(tnew[start_index:], sinfunc(Tparams2, tnew[start_index:]), 'g-', label='T-Fit-Top')
		...
		#SineWave+LinearRamp Method
			...
			if input == "y":
				...
				Tresults = minimize(sinfunc, Tparams, args=(tnew[start:], Tnew[start:]), method="leastsq")
		...
		#---------------------------------------------------------------------------------------------------------------------
			...
			if input == "y":
				...
				for i in arange(1,number_p_heat):
					...
					if phasediff < 0.0:
						phasediff = phasediff+2*pi
					p_heat[i-1,0] = (tnew[start_index+((i-1)*satzlaenge)]*Tfit_heat[4])+(((tnew[start_index+((i-1)*satzlaenge)]-tnew[start_index+(i*satzlaenge)])/2)*Tfit_heat[4])+Tfit_heat[3]			#Spalte1 - Temperatur
