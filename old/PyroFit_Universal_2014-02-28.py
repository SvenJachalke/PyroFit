...
# General Settings----------------------------------------------------------------------------------------------------------
# Plot Settings-------------------------------------------------------------------------------------------------------------
matplotlib.rcParams['legend.fancybox'] = True       #plot fancy box (round corners)
label_size = '15'									#font size of x,y labels in plot
...
def saving_figure(filename):
	"""
	saves figure with individual filename and print on console
	input:	filename [str]
	output: none
	"""
	print "--------------------------------"
	print "...saving figure"
	savefig(filename, dpi=set_dpi, transparent=transparency_flag)
	return None
...
#-------------------------------------------------------------------------------------------------------------------------------------
	...
	if HV_status == "Off":
		...
		if T_profile == "Thermostat":
			...
			#saving figure
			saving_figure(date+"_"+samplename+"_"+T_profile+".png")
		...
		#SineWave Method
			...
			if input == "y":
				...
				Tparams_down.add('amp', value=start_parameters[0], min=0.1, max=20)
				Tparams_down.add('freq', value=start_parameters[1], min=1e-5, max=0.1, vary=False)
				Tparams_down.add('phase', value=pi/2, min=-2*pi, max=+2*pi)
				Tparams_down.add('offs', value=start_parameters[2], min=273., max=423.)
				Tparams_down.add('slope', value=start_parameters[3])

				#fit of bottom temperature
				Tresults = minimize(sinfunc, Tparams_down, args=(tnew[start_index:], Tnew_down[start_index:]), method="leastsq")
				Tfit_down, Terror_low = extract_fit_relerr_params(Tparams_down)		#write fit params to Tfit-list
				...
				ax1.legend(loc=1)
				ax2.legend(loc=4)
				#text for information box
				box_text = r"Area: "+str(area)+ r" $\mathrm{m^2}$"+"\n"+ r"I-Amp: "+str(Ifit[0])+r" A"+"\n"+ r"T-Amp: "+str(Tfit_down[0])+r" K"+"\n"+r"Phase-Diff.: "+str(degrees(phasediff))+"$^{\circ}$"+"\n"+r"pyroel. Coeff.: "+str(pyro_koeff*1e6)+r" $\mathrm{\mu C/Km^2}$"
				figtext(0.15,0.12,box_text,fontdict=None,bbox=dict(facecolor=facecolor_legends, alpha=0.5))
				...
				log.write("T-Fit Down:\tA=%f K\t\tf=%f Hz\tp=%f\t\tOffs=%f K\t\tb=%f K/s\n" % (Tfit_down[0],Tfit_down[1],Tfit_down[2],Tfit_down[3],Tfit_down[4]))
				if temp_filter_flag	== False:
					log.write("T-Fit Top: \tA=%f K\t\tf=%f Hz\tp=%f\t\tOffs=%f K\t\t\tb=%f K/s\n" % (Tfit_high[0],Tfit_high[1],Tfit_high[2],Tfit_high[3],Tfit_high[4]))
		...
		#SineWave+LinearRamp Method
			...
			for i in arange(0,len(Tnew_down)-1):
				if Tnew_down[i] < start_parameters[5]-1:
					limit = i
			...
			ax1 = subplot(111)
			ax2 = ax1.twinx()
			title(samplename+"_SineWave+LinRamp", size=title_size)
			...
			ax1.grid(b=None, which='major', axis='both', color='grey', linewidth=1)
			ax1.tick_params(axis='y', colors='blue')
			line1 = ax1.plot(tnew[start_index:], Tnew_down[start_index:], 'bo', label="T meas. (Down)")
			ax1.autoscale(enable=True, axis='y', tight=None)
			ax1.legend(title="temperatures", loc='upper right')
			if temp_filter_flag == False:
				line2 = ax1.plot(tnew[start_index:-5], Tnew_top[start_index:], 'go', label="T meas. (Top)")
				ax1.autoscale(enable=True, axis='y', tight=None)
				ax1.legend(title="temperatures", loc='upper right')
			...
			if input == "y":
				...
				Tparams_down = Parameters()
				Tparams_down.add('amp', value=start_parameters[0], min=0.1, max=40.0)
				Tparams_down.add('freq', value=start_parameters[1], min=1e-5, max=0.1, vary=False)
				Tparams_down.add('phase', value=0.1, min=-pi, max=pi)
				Tparams_down.add('offs', value=start_parameters[2], min=0.0)
				Tparams_down.add('slope', value=start_parameters[3])

				#perform fit
				Tresults = minimize(sinfunc, Tparams_down, args=(tnew[start_index:limit], Tnew_down[start_index:limit]), method="leastsq")
